...
const SGame = (function () {
    ...
    class Scene {
        ...
        worldAddDemo(_demo) {
            ...
            if (_demo.type_name == "TextureDemo" ||
                ...
                bodyDef.type = _demo.box2d_body_type;
                // ローテンションは任意にさせる予定
    ...
    // デモ基底クラス
    class Demo {
        constructor(_name, _res_idx = null, _width = V_WIDTH, _height = V_HEIGHT, _pos_x = 0, _pos_y = 0, _scale_x = 1, _scale_y = 1, _alpha = 1) {
            // デモ名
            this.name = _name;
            // 更新処理関数
            this.update = null;
            ...
            // Box2dの剛体属性（初期値は静的）
            this.box2d_body_type = sBox2dBodyType.DynamicBody;
        }
        // サウンド再生
        startSound(_media) {
            try {
    ...
    class TextureDemo extends Demo {
        constructor(_name, _res_idx, _width = V_WIDTH, _height = V_HEIGHT, _pos_x = 0, _pos_y = 0, _scale_x = 1, _scale_y = 1, _alpha = 1) {
            // 親クラス呼び出し
            super(_name, _res_idx, _width, _height, _pos_x, _pos_y, _scale_x, _scale_y, _alpha);
            // タイプ名
    ...
    class ChipTextureDemo extends Demo {
        constructor(_name, _res_idx, _start_idx, _idx_num_x, _idx_num_y, _width = V_WIDTH, _height = V_HEIGHT, _pos_x = 0, _pos_y = 0, _scale_x = 1, _scale_y = 1, _alpha = 1) {
            // 親クラス呼び出し
            super(_name, _res_idx, _width, _height, _pos_x, _pos_y, _scale_x, _scale_y, _alpha);
            // 現在のインデックス
            this.now_idx = _start_idx;
            // 横サイズ
